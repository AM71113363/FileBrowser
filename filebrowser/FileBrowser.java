...
public class FileBrowser
{
	...
	loadInts();
	//use backup,on screen rotate the app restarsts
	...
	for(i=0;i<len;i++)
	{
	    ...
	    View hy=makeFrame(p.getName(), p.isFile(), p.isBlur(),i);
	...
	if(dir==null) //without this the app will crash if it doesn't have storage permissions
	  return false; //unless you catch the error ofcourse.
	DATABASE.ResetList();
	//add back element
	BAG p=new BAG("../");
	p.isFile(false);
	p.isBlur(true);//only files have isBlur(true),I'm using it as a flag to find the click on BACK button
	DATABASE.add(p);
	...
	if(dir.length>1) //whats the point of sorting a single item
	{
	...
	}else{ imgv.setBackgroundColor(Color.YELLOW); }
	...
	public void handleMessage(Message ms)
	{
	    switch(ms.what)
	    {
	        case CODE_EXPLORE_NO_BACKUP:{ EXPLORE(false); }break;
	        case CODE_EXPLORE_USE_BACKUP:{ EXPLORE(true); }break;
		case CODE_DONE:
		{
		...
		case CODE_CANCEL:{ call.DONE("",true); }break;
		default:
		  break;
	    }
	}
